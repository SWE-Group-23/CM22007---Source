...
class BlockUserRPCServer(rpcs.RPCServer):
    """
    Subclass of RPCServer
    """
    def __init__(self, rabbitmq_user: str, rabbitmq_pass: str, *, rpc_prefix="block-user-rpc"):
        super().__init__(rabbitmq_user, rabbitmq_pass, rpc_prefix)

    def _block_chat(self, chat_id):
        """
        Attempts to add a message to the messages table
        """
        try:
            models.Chats.objects(chat_id=chat_id).update(blocked=True)
            return rpcs.response(200, {"message": "Success"})
        except ConnectionError as e:
            logging.error("[DB ERROR] %s", e, exc_info=True)
            return rpcs.response(400, {"message": "Error blocking user"})
